...
import { Languages, Loader2, Sparkles, Check, AlertCircle } from 'lucide-react';
...
export default function TranslationButton({
  ...
  const handleTranslate = async () => {
    ...
    if (!isAvailable) {
      setError(
        'Translation service not available. Please check your configuration.'
      );
      setTranslationStatus('error');
      setTimeout(() => setTranslationStatus('idle'), 3000);
      return;
          ...
          <TooltipTrigger asChild>
            <Button
              ...
              className={`
                ...
                ${translationStatus === 'success' ? 'border-primary text-primary' : ''}
                ${className}
              `}
            >
              {getButtonContent()}
      ...
      {targetLanguages.map(targetLang => (
        <TranslationButton
          ...
          onTranslated={translatedText =>
            onTranslated(targetLang, translatedText)
          }
          disabled={disabled}
          size="sm"
          variant="outline"
    ...
    try {
      for (let i = 0; i < translationTasks.length; i++) {
        ...
        try {
          ...
          console.error(
            `Translation failed for ${task.fieldKey} -> ${task.targetLang}:`,
            error
          );
