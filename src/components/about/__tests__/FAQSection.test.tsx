...
describe('FAQSection', () => {
  ...
  it('renders FAQ section with title', () => {
    render(<FAQSection faqs={mockFAQs} title="Test FAQs" locale="en" />);
  ...
  it('renders FAQs in simple accordion layout', () => {
    ...
    // Use getAllByText since questions might appear multiple times
    expect(screen.getAllByText('What is your service?').length).toBeGreaterThan(
      0
    );
    expect(
      screen.getAllByText('How much do you charge?').length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText('What equipment do you use?').length
    ).toBeGreaterThan(0);
  });

  it('handles service FAQ structure correctly', () => {
    render(<FAQSection faqs={mockServiceFAQs} title="Test FAQs" locale="en" />);
  ...
  it('handles service FAQ structure with Spanish locale', () => {
    render(<FAQSection faqs={mockServiceFAQs} title="Test FAQs" locale="es" />);

    // Use getAllByText since the question might appear multiple times
    expect(screen.getAllByText('¿Cuál es tu servicio?').length).toBeGreaterThan(
      0
    );
  });
  ...
  it('returns null when no FAQs provided', () => {
    const { container } = render(
      <FAQSection faqs={[]} title="Test FAQs" locale="en" />
    );
