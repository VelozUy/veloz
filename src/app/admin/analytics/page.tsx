...
interface DashboardMetrics {
  ...
  trafficMediumBreakdown?: Record<string, number>;
  utmSourceBreakdown?: Record<string, number>;
  utmCampaignBreakdown?: Record<string, number>;
  referrerDomainBreakdown?: Record<string, number>;
}
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      summaries.forEach(summary => {
        ...
        // Aggregate traffic breakdowns if present
        if ((summary as any).trafficSourceBreakdown) {
          Object.entries(
            (summary as any).trafficSourceBreakdown as Record<string, number>
          ).forEach(([source, count]) => {
            aggregatedMetrics.trafficSourceBreakdown![source] =
              (aggregatedMetrics.trafficSourceBreakdown![source] || 0) + count;
          });
        }
        if ((summary as any).trafficMediumBreakdown) {
          Object.entries(
            (summary as any).trafficMediumBreakdown as Record<string, number>
          ).forEach(([medium, count]) => {
            aggregatedMetrics.trafficMediumBreakdown![medium] =
              (aggregatedMetrics.trafficMediumBreakdown![medium] || 0) + count;
          });
        }
        if ((summary as any).utmSourceBreakdown) {
          Object.entries(
            (summary as any).utmSourceBreakdown as Record<string, number>
          ).forEach(([utm, count]) => {
            aggregatedMetrics.utmSourceBreakdown![utm] =
              (aggregatedMetrics.utmSourceBreakdown![utm] || 0) + count;
          });
        }
        if ((summary as any).utmCampaignBreakdown) {
          Object.entries(
            (summary as any).utmCampaignBreakdown as Record<string, number>
          ).forEach(([camp, count]) => {
            aggregatedMetrics.utmCampaignBreakdown![camp] =
              (aggregatedMetrics.utmCampaignBreakdown![camp] || 0) + count;
          });
        }
        if ((summary as any).referrerDomainBreakdown) {
          Object.entries(
            (summary as any).referrerDomainBreakdown as Record<string, number>
          ).forEach(([dom, count]) => {
            aggregatedMetrics.referrerDomainBreakdown![dom] =
              (aggregatedMetrics.referrerDomainBreakdown![dom] || 0) + count;
          });
        }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Monitorea el rendimiento de tus proyectos y la engagement de
                  los usuarios
                </p>
              </div>
              <Button
            ...
            <TabsContent value="traffic" className="space-y-3">
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-sm">
                    Origen, medio, campañas (UTM) y dominios de referencia
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-muted-foreground">
                        {metrics.trafficSourceBreakdown &&
                          Object.entries(metrics.trafficSourceBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 10)
                            .map(([k, v]) => (
                              <li key={k}>
                    ...
                    <div>
                      <h4 className="font-semibold mb-1.5 text-sm">Medio</h4>
                      <ul className="text-xs text-muted-foreground">
                        {metrics.trafficMediumBreakdown &&
                          Object.entries(metrics.trafficMediumBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 10)
                            .map(([k, v]) => (
                              <li key={k}>
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-muted-foreground">
                        {metrics.utmSourceBreakdown &&
                          Object.entries(metrics.utmSourceBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 10)
                            .map(([k, v]) => (
                              <li key={k}>
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-muted-foreground">
                        {metrics.utmCampaignBreakdown &&
                          Object.entries(metrics.utmCampaignBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 10)
                            .map(([k, v]) => (
                              <li key={k}>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <ul className="text-xs text-muted-foreground columns-2">
                        {metrics.referrerDomainBreakdown &&
                          Object.entries(metrics.referrerDomainBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 20)
                            .map(([k, v]) => (
                              <li key={k}>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {crewAnalytics.topPerformingCrew.map(
                            (crew, index) => (
                              <div
                                key={crew.id}
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">
                                      {crew.projectsCompleted} proyectos
                                      completados
                                    </div>
                                  </div>
                                </div>
