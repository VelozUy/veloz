...
import { Users, Globe, Calendar, Camera, Video, FileText, Settings, Eye, Edit, Copy, Trash2 } from 'lucide-react';
...
export default function TemplatesCategoriesPage() {
  ...
  const getCategoryStats = (categoryId: string) => {
    const categoryTemplates = templates.filter(template => 
      template.eventType?.toLowerCase() === categoryId
    );
    ...
    const totalTasks = categoryTemplates.reduce((sum, template) => sum + template.tasks.length, 0);
    const avgTasks = Math.round(totalTasks / categoryTemplates.length);
    const highPriorityTasks = categoryTemplates.reduce((sum, template) => 
      sum + template.tasks.filter(task => task.priority === 'high').length, 0
    );
  ...
  const getOverallStats = () => {
    ...
    const avgTasks = totalTemplates > 0 ? Math.round(totalTasks / totalTemplates) : 0;
    const activeCategories = new Set(templates.map(t => t.eventType?.toLowerCase())).size;
  ...
  return (
    <AdminLayout title="Plantillas por Categoría">
      <div className="space-y-6">
                    ...
                    <Card key={category.id} className="hover:shadow-md transition-shadow">
                      <CardHeader className="pb-3">
                        <div className="flex items-center space-x-2">
                          ...
                          <CardTitle className="text-body-lg">{category.name}</CardTitle>
                        </div>
                      ...
                      <CardContent>
                        <div className="flex gap-2 flex-wrap">
                          <Badge variant="outline" className="text-xs">
                            {stats.templateCount} plantilla{stats.templateCount !== 1 ? 's' : ''}
                          </Badge>
                          <Badge variant="outline" className="text-xs">
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <th className="text-left p-3 font-medium">Tareas Promedio</th>
                      <th className="text-left p-3 font-medium">Duración Promedio</th>
                      <th className="text-left p-3 font-medium">Alta Prioridad</th>
                    ...
                    {eventCategories.map((category) => {
                      ...
                      return (
                        <tr key={category.id} className="border-b border-border hover:bg-muted/50">
                          ...
                          <td className="p-3">
                            <Badge variant="outline">{stats.templateCount}</Badge>
                          </td>
                          <td className="p-3">
                            <div className="flex items-center gap-2">
                              <span className="font-medium">{stats.avgTasks}</span>
                              <span className="text-sm text-muted-foreground">tareas</span>
                            </div>
                          </td>
                          <td className="p-3">
                            <span className="text-sm">{stats.avgDuration} días</span>
                          </td>
                          <td className="p-3">
                            {stats.highPriorityTasks > 0 ? (
                              <Badge variant="destructive" className="text-xs">
                          ...
                          <td className="p-3">
                            <Badge variant={stats.templateCount > 0 ? "secondary" : "outline"}>
                          ...
                          <td className="p-3">
                            <div className="flex items-center gap-1">
                              ...
                              <Button variant="ghost" size="sm" title="Crear plantilla">
              ...
              <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2">{overallStats.activeCategories}</span>
                  </div>
                </div>
              </div>
            </TabsContent>
        ...
        <div className="bg-card rounded-lg border p-6">
          <h2 className="text-lg font-semibold mb-4">Plantillas Filtradas por Categoría</h2>
          <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
            ...
            <TabsContent value="all" className="mt-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-muted-foreground">
                  Mostrando {templates.length} plantillas de todas las categorías
                </p>
              </div>
                ...
                <TabsContent key={category.id} value={category.id} className="mt-6">
                  <div className="mb-4">
                    ...
                    <div className="flex gap-2 text-sm text-muted-foreground">
                      <span>{stats.templateCount} plantilla{stats.templateCount !== 1 ? 's' : ''}</span>
                      <span>•</span>
                  ...
                  {filteredTemplates.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Crea la primera plantilla para {category.name.toLowerCase()}</p>
                    </div>
                  )}
                </TabsContent>
              );
        ...
        {/* Category Statistics */}
        <div className="bg-card rounded-lg border p-6">
          <h2 className="text-lg font-semibold mb-4">Estadísticas por Categoría</h2>
            ...
            {eventCategories.slice(0, 4).map((category) => {
              ...
              return (
                <div key={category.id} className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    plantilla{stats.templateCount !== 1 ? 's' : ''} activa{stats.templateCount !== 1 ? 's' : ''}
                  </p>
                  {stats.avgTasks > 0 && (
