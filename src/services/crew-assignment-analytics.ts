...
export class CrewAssignmentAnalyticsService {
  ...
  async getCrewMemberAssignmentMetrics(
    ...
    try {
      ...
      const ratings = projects
        ...
        .filter(r => r > 0);
      const averageProjectRating = ratings.length > 0 
        ? ratings.reduce((sum, rating) => sum + rating, 0) / ratings.length 
        : 0;

      // Calculate team collaboration metrics
      const soloProjects = projects.filter(p => (p.crewMembers?.length || 0) === 1).length;
      const teamProjects = projects.filter(p => (p.crewMembers?.length || 0) > 1).length;
      const averageTeamSize = projects.length > 0 
        ? projects.reduce((sum, p) => sum + (p.crewMembers?.length || 1), 0) / projects.length 
        : 1;

      // Find preferred partners
      const partnerMap = new Map<string, { projects: number; successful: number }>();
      projects.forEach(project => {
        if ((project.crewMembers?.length || 0) > 1) {
          project.crewMembers?.forEach((memberId: string) => {
            if (memberId !== crewMemberId) {
              const current = partnerMap.get(memberId) || { projects: 0, successful: 0 };
              current.projects += 1;
      ...
      const preferredPartners = await Promise.all(
        Array.from(partnerMap.entries()).map(async ([partnerId, stats]) => {
          ...
          const partnerName = partnerDoc.exists() 
            ? partnerDoc.data().name?.es || partnerDoc.data().name?.en || 'Unknown'
            : 'Unknown';
          ...
          return {
            ...
            successRate: stats.projects > 0 ? (stats.successful / stats.projects) * 100 : 0,
          };
        })
      );

      // Calculate category performance
      const categoryMap = new Map<string, { projects: number; successful: number; ratings: number[] }>();
      projects.forEach(project => {
        const category = project.eventType || 'other';
        const current = categoryMap.get(category) || { projects: 0, successful: 0, ratings: [] };
        current.projects += 1;
        if (project.status === 'delivered') {
          current.successful += 1;
        }
      ...
      const onTimeDeliveries = projects.filter(p => {
        ...
        const endDate = p.timeline?.endDate?.toDate?.() || new Date(p.timeline?.endDate);
        const completionDate = p.completedAt?.toDate?.() || new Date(p.completedAt);
        return completionDate <= endDate;
      }).length;

      const lateDeliveries = completedAssignments - onTimeDeliveries;
      const onTimeDeliveryRate = completedAssignments > 0 
        ? (onTimeDeliveries / completedAssignments) * 100 
        : 0;

      // Calculate average completion time
      const completionTimes = projects
        .filter(p => p.status === 'delivered' && p.completedAt && p.timeline?.startDate)
        .map(p => {
          const startDate = p.timeline?.startDate?.toDate?.() || new Date(p.timeline?.startDate);
          const completionDate = p.completedAt?.toDate?.() || new Date(p.completedAt);
          return (completionDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24);
        });
      ...
      const efficiencyScore = Math.round(
        (successRate * 0.4) + (onTimeDeliveryRate * 0.3) + (averageProjectRating * 10 * 0.3)
      );

      // Calculate client satisfaction metrics
      const positiveReviews = projects.filter(p => 
        p.clientSatisfaction?.rating && p.clientSatisfaction.rating >= 4
      ).length;
      const negativeReviews = projects.filter(p => 
        p.clientSatisfaction?.rating && p.clientSatisfaction.rating <= 2
      ).length;
      ...
      const metrics: CrewAssignmentMetrics = {
        ...
        teamCollaboration: {
          ...
          teamProjects,
          averageTeamSize,
          preferredPartners: preferredPartners.sort((a, b) => b.projectsTogether - a.projectsTogether),
        },
        categoryPerformance,
        timePerformance: {
          onTimeDeliveries,
      ...
      console.error('Error fetching crew assignment metrics:', error);
      return { success: false, error: 'Failed to fetch crew assignment metrics' };
    }
  }
  ...
  async getTeamCollaborationMetrics(
    ...
    try {
      ...
      const filteredProjects = projects.filter(p => teamMemberIds.every(id => p.crewMembers?.includes(id)));

      const totalProjects = filteredProjects.length;
      const successfulProjects = filteredProjects.filter(p => p.status === 'delivered').length;
      ...
      // Get project types
      const projectTypes = [...new Set(filteredProjects.map(p => p.eventType || 'other'))];
      ...
      return { success: false, error: 'Failed to fetch team collaboration metrics' };
    }
  }
   ...
   */
  async getCrewAssignmentAnalytics(): Promise<ApiResponse<CrewAssignmentAnalytics>> {
    try {
      const db = await getFirestoreService();
      if (!db) throw new Error('Firestore not available');
      ...
      const totalAssignments = projects.reduce((sum, p) => sum + (p.crewMembers?.length || 0), 0);
      ...
      const averageSuccessRate = crewPerformance.length > 0 
        ? crewPerformance.reduce((sum, crew) => sum + crew.successRate, 0) / crewPerformance.length 
        : 0;

      // Find best team combinations
      const teamCombinations = new Map<string, { projects: number; successful: number }>();
      projects.forEach(project => {
        if ((project.crewMembers?.length || 0) > 1) {
          const sortedMembers = project.crewMembers?.sort() || [];
          const teamKey = sortedMembers.join('-');
          const current = teamCombinations.get(teamKey) || { projects: 0, successful: 0 };
          current.projects += 1;
          if (project.status === 'delivered') {
            current.successful += 1;
          }
      ...
      const categoryMap = new Map<string, { projects: number; successful: number; crewMembers: Set<string> }>();
      projects.forEach(project => {
        const category = project.eventType || 'other';
        const current = categoryMap.get(category) || { projects: 0, successful: 0, crewMembers: new Set() };
        current.projects += 1;
        if (project.status === 'delivered') {
          current.successful += 1;
        }
        project.crewMembers?.forEach((crewId: string) => current.crewMembers.add(crewId));
        categoryMap.set(category, current);
      });
      ...
      const categoryAnalysis = Array.from(categoryMap.entries()).map(([category, stats]) => ({
        ...
        averageSuccessRate: stats.projects > 0 ? (stats.successful / stats.projects) * 100 : 0,
      ...
      console.error('Error fetching crew assignment analytics:', error);
      return { success: false, error: 'Failed to fetch crew assignment analytics' };
    }
  }
}

// Export singleton instance
export const crewAssignmentAnalyticsService = new CrewAssignmentAnalyticsService(); 