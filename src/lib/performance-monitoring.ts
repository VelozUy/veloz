...
export function generatePerformanceReport(): PerformanceMetrics {
  ...
  if (lcpEntries.length > 0) {
    const lastLCP = lcpEntries[lcpEntries.length - 1] as PerformanceEntry & { value: number };
  ...
  if (fcpEntries.length > 0) {
    const lastFCP = fcpEntries[fcpEntries.length - 1] as PerformanceEntry & { value: number };
...
export function logPerformanceSummary(): void {
  ...
  console.log('LCP:', metrics.lcp, 'ms', metrics.lcp <= PERFORMANCE_TARGETS.lcp ? '✅' : '❌');
  console.log('TBT:', metrics.tbt, 'ms', metrics.tbt <= PERFORMANCE_TARGETS.tbt ? '✅' : '❌');
  console.log('Speed Index:', metrics.speedIndex, 'ms', metrics.speedIndex <= PERFORMANCE_TARGETS.speedIndex ? '✅' : '❌');
  console.log('FCP:', metrics.fcp, 'ms', metrics.fcp <= PERFORMANCE_TARGETS.fcp ? '✅' : '❌');
  ...
  console.log('Speed Index improvement:', improvements.speedIndex.toFixed(1), '%');
}
